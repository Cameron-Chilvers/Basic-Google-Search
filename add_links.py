...
class Neo4jConnection:
    ...
    def add_links_to_relationships(self, batch):
        with self.__driver.session() as session:
            session.write_transaction(self.__add_links_to_relationships_batch, batch)

    @staticmethod
    def __add_links_to_relationships_batch(tx, batch):
        query = """
            UNWIND $batch AS line
            MERGE (a:Website {id: line[0]}), (b:Website {id: line[1]})
            MERGE (a)-[:LINKS_TO]->(b)
            RETURN a, b
            """
        tx.run(query, batch=batch)
...
def add_links_to_relationships(filepath, conn: Neo4jConnection, batch_size=500):
    start = time()
    batch_count = 0
    ...
    for line in lines:
        ...
        batch.append((str(source_id), str(target_id)))

        if len(batch) >= batch_size:
            print("trying to insert batch ", batch_count)
            try:
                conn.add_links_to_relationships(batch)
            except Exception as e:
...
print(conn.execute_query("MATCH (n:Website {id: '0'}) RETURN n"))
if input("Continue?") == 1:
    quit()

links_file_path = r"nodes-links\links.txt"
add_links_to_relationships(links_file_path, conn)
